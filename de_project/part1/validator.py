...
class TaxiValidator:
    ...
    def validate(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info(f"Starting validation on {original_count} rows.")
        ...
        logger.info(f"Validation complete. Removed {removed} invalid rows. {len(df)} rows remaining.")
        return df
    ...
    def _drop_mandatory_nulls(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if dropped:
            logger.warning(f"Dropped {dropped} rows with nulls in mandatory columns.")
        return df

    def _validate_datetime_columns(self, df: pd.DataFrame) -> pd.DataFrame:
        for col in ["tpep_pickup_datetime", "tpep_dropoff_datetime"]:
            if not pd.api.types.is_datetime64_any_dtype(df[col]):
                logger.warning(f"Column '{col}' is not datetime — attempting conversion.")
                df[col] = pd.to_datetime(df[col], errors="coerce")
        before = len(df)
        df = df.dropna(subset=["tpep_pickup_datetime", "tpep_dropoff_datetime"])
        dropped = before - len(df)
        if dropped:
            logger.warning(f"Dropped {dropped} rows with unparseable datetime values.")
        return df
    ...
    def _validate_pickup_before_dropoff(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        bad = mask.sum()
        if bad:
            logger.warning(f"Dropped {bad} rows where pickup >= dropoff datetime.")
            df = df[~mask]
        return df

    def _validate_passenger_count(self, df: pd.DataFrame) -> pd.DataFrame:
        mask = (df["passenger_count"] < 1) | (df["passenger_count"] > self.MAX_PASSENGER_COUNT)
        bad = mask.sum()
        if bad:
            logger.warning(f"Dropped {bad} rows with invalid passenger_count (must be 1–{self.MAX_PASSENGER_COUNT}).")
            df = df[~mask]
        return df

    def _validate_trip_distance(self, df: pd.DataFrame) -> pd.DataFrame:
        mask = (df["trip_distance"] < 0) | (df["trip_distance"] > self.MAX_TRIP_DISTANCE)
        bad = mask.sum()
        if bad:
            logger.warning(f"Dropped {bad} rows with invalid trip_distance (must be 0–{self.MAX_TRIP_DISTANCE}).")
            df = df[~mask]
        return df
    ...
    def _validate_location_ids(self, df: pd.DataFrame) -> pd.DataFrame:
        for col in ["PULocationID", "DOLocationID"]:
            ...
            bad = mask.sum()
            if bad:
                logger.warning(f"Dropped {bad} rows with invalid {col} (must be 1–265).")
                df = df[~mask]
        return df
    ...
    def _validate_payment_type(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        bad = mask.sum()
        if bad:
            logger.warning(f"Dropped {bad} rows with invalid payment_type (valid: {self.VALID_PAYMENT_TYPES}).")
            df = df[~mask]
        return df
    ...
    def _validate_fare_amount(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        bad = mask.sum()
        if bad:
            logger.warning(f"Dropped {bad} rows with invalid fare_amount (must be 0–{self.MAX_FARE}).")
            df = df[~mask]
        return df
    ...
    def _validate_total_amount(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        bad = mask.sum()
        if bad:
            logger.warning(f"Dropped {bad} rows with invalid total_amount (must be 0–{self.MAX_FARE}).")
            df = df[~mask]
        return df
    ...
    def _validate_non_mandatory_columns(self, df: pd.DataFrame) -> None:
        for col in self.NON_MANDATORY_COLUMNS:
            if col not in df.columns:
                logger.info(f"Non-mandatory column '{col}' not present in dataset.")
                continue

            null_count = df[col].isnull().sum()
            if null_count:
                logger.info(f"Non-mandatory column '{col}' has {null_count} null values (allowed).")
            ...
            if col == "store_and_fwd_flag":
                ...
                if len(bad):
                    logger.warning(f"'{col}' has {len(bad)} unexpected values (expected Y/N).")

            elif col == "RatecodeID":
                bad = df[col].dropna()
                bad = bad[~bad.isin(self.VALID_RATECODEID)]
                if len(bad):
                    logger.warning(f"'{col}' has {len(bad)} values outside valid range {self.VALID_RATECODEID}.")

            elif col in ("tip_amount", "tolls_amount", "extra", "Airport_fee",
                         "congestion_surcharge", "cbd_congestion_fee"):
                neg = (df[col].dropna() < 0).sum()
                if neg:
                    logger.warning(f"'{col}' has {neg} negative values.")